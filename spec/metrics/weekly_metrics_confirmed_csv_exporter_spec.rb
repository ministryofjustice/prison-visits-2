...
RSpec.describe WeeklyMetricsConfirmedCsvExporter do
  ...
  describe '#to_csv' do
    let(:prison1) { FactoryGirl.create(:prison, name: 'A Prison') }
    let(:prison2) { FactoryGirl.create(:prison, name: 'B Prison') }
    let(:prison3) { FactoryGirl.create(:prison, name: 'C Prison') }
    let(:week_ago) { 1.week.ago }
    let(:two_weeks_ago) { 2.weeks.ago }

    let!(:recent_confirmed_visit) do
      FactoryGirl.create(:booked_visit, prison: prison3)
    end
    let!(:week_old_confirmed_visit) do
      FactoryGirl.create(:booked_visit, prison: prison1, created_at: week_ago)
    end
    let!(:two_week_old_confirmed_visit) do
      FactoryGirl.create(:booked_visit, prison: prison2, created_at: two_weeks_ago)
    end
