...
RSpec.feature 'Processing a request - Acceptance with the contact list enabled', :expect_exception, :js do
  ...
  context 'when accepting a booking but contact list fails', vcr: { cassette_name: :process_contact_list_fails } do
    ...
    it 'is expected that the contact list is not available' do
      ...
      expect(page).to have_css('form .notice', text: "We can’t show the contact list right now. Please check all visitors in NOMIS")
    end
  end
end
